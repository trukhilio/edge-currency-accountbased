...
var _queries = require('@fioprotocol/fiosdk/lib/transactions/queries');
...
,
  ...
  async syncNetwork(opts) {
    const fioPrivateKeys = _fioTypes.asFioPrivateKeys.call(void 0, _optionalChain([opts, 'optionalAccess', _39 => _39.privateKeys]))
    let isChanged = false
    ...
    const checkFioRequests = async (
      ...
      const decryptedReqs = _nullishCoalesce(decoder.decrypt({
    ...
    await checkFioRequests(
      'PENDING',
      new (0, _queries.PendingFioRequests)(this.walletInfo.keys.publicKey)
    )
    await checkFioRequests(
      'SENT',
      new (0, _queries.SentFioRequests)(this.walletInfo.keys.publicKey)
    )

    if (isChanged) this.localDataDirty()

    const obtDecoder = new (0, _queries.GetObtData)(this.walletInfo.keys.publicKey)
    ...
    const decryptedObtData =
      _nullishCoalesce(obtDecoder.decrypt({
  ...
  async broadcastTx(
    ...
    if (_optionalChain([otherParams, 'access', _40 => _40.action, 'optionalAccess', _41 => _41.name]) == null) {
      throw new Error(
